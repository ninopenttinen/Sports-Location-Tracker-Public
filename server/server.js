  ...
  try {
    results = await pool.query(
      `SELECT 
                                        ...
                                        ROUND((ST_Distance(coordinates, ST_GeogFromText('SRID=4326;POINT(${
                                          longitude + ' ' + latitude
                                        })'))/1000)::NUMERIC(8,2),2) AS distance
                                    FROM locations 
                                    WHERE
                                        lower(type) LIKE lower($1) AND
                                        ST_DWithin(coordinates, ST_GeogFromText('SRID=4326;POINT(${
                                          longitude + ' ' + latitude
                                        })'), $2)
...
app.post(basepath + '/api/locations', async (req, res) => {
  ...
  if (
    location.length === 0 ||
    location.length > 50 ||
    !location.match('^$|^[A-Za-z0-9-äöåÄÖÅ() ]+$') ||
    type.length === 0 ||
    type.length > 50 ||
    !type.match('^$|^[A-Za-z0-9-äöåÄÖÅ ]+$') ||
    description.length > 250 ||
    !description.match('^$|^[A-Za-z0-9-äöåÄÖÅ()/.,!?_ ]+$') ||
    rating < 0 ||
    rating > 5 ||
    latitude.length === 0 ||
    longitude.length === 0
  ...
  try {
    await pool.query(
      `INSERT INTO locations (location, type, description, rating, coordinates, added)
                          VALUES
                          ($1, $2, $3, $4, ST_GeogFromText('SRID=4326;POINT(${
                            longitude + ' ' + latitude
                          })'), NOW());`,
