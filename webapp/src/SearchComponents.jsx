...
export class SearchMenu extends Component {
    ...
    handleValueChange = (event, key) => {
            this.setState({ [key]: event.target.value },
                () => this.validate(key));  // callback to validate the input, if the input doesn't comply with the set rules
    ...
    validate = (key) => {
        ...
        if (key === 'searchType')
            if (searchType.length > 50 || !searchType.match("^$|^[A-Za-z0-9-äöåÄÖÅ ]+$"))
                this.setState({ error: { ...error, searchTypeError: true } });
            else
                this.setState({ error: { ...error, searchTypeError: false } });

        if (key === 'searchDistanceKm' || key === 'searchDistanceM')
            if (searchDistanceKm.length === 0 || searchDistanceM.length === 0 ||
                !searchDistanceKm.match("^$|^[0-9]+$") || !searchDistanceM.match("^$|^[0-9]+$"))
    ...
    handleSetCoordinates = (select) => {
        ...
        if (select === 'from_map') {
            startMapSelection();
            setState({ error: { ...this.state.error, searchCoordinateError: false } });
        }
        else
            ...
            if (navigator.geolocation) {
                ...
                .then(function(PermissionStatus) {
                    ...
                    if (PermissionStatus.state === 'granted') {
                        ...
                        setState({ statusMsg: 3 });
                });
            } else
                setState({ statusMsg: 4 });
    };
    ...
    handleSearch = () => {
        ...
        if (searchDistanceKm.length === 0 || !searchDistanceKm.match("^$|^[0-9]+$") ||
            searchDistanceM.length === 0 || !searchDistanceM.match("^$|^[0-9]+$"))
        ...
        if (setTypeError || setDistanceError || setCoordinateError)
            ...
            this.setState({
                ...
                axios.get(`http://localhost:9000/api/locations?type=${searchType}&latitude=${latitude}&longitude=${longitude}&distance=${parseInt(searchDistanceKm)*1000 + parseInt(searchDistanceM)}`)
                .then(res => {
                    this.setState({ 
                        loading: false,
                        statusMsg: ''
                    }, () => saveResults(res.data) );   // callback to parent component to save the results to its state, so they can be used in other views
                })
                .catch(err => {
                    console.log(err);
                    this.setState({ 
    ...
    render() {
        const { searchType, searchDistanceKm, searchDistanceM, statusMsg, loading } = this.state;
        const { latitude, longitude, selectingFromMap, enableMapCentering, handleCheckboxChange } = this.props;
        const { searchTypeError, searchCoordinateError, searchDistanceError } = this.state.error;
        const { handleValueChange, handleSetCoordinates, handleReset, handleSearch } = this;
                ...
                <div id="search-menu">
                    ...
                    <div id="input-fields">
                        <section>
                            ...
                            <input id="search-type" 
                                ...
                                type="text" autoComplete="off" spellCheck="false" placeholder="Leave empty to search all locations"
                                value={searchType}
                        ...
                        <section>
                            <div style={{display: "flex", alignItems: "center"}}>
                                <label htmlFor="search-coordinates-btn-group">Your location</label>
                            ...
                            <span id="search-coordinates-btn-group" style={{display: "flex", alignItems: "center"}}>
                                <button id={(selectingFromMap) ? "search-coordinates-left-active" : "search-coordinates-left"}
                                ...
                                <button id="search-coordinates-right"
                                    onClick={handleSetCoordinates}>
                                Use current
                                </button>
                            </span>
                            <p style={{paddingLeft: "1%", marginBottom: "2%"}}>Lat:&nbsp;{latitude}</p>
                            <p style={{paddingLeft: "1%", marginTop: "0"}}>Lng:&nbsp;{longitude}</p>
                        </section>
                        ...
                        <section>
                            ...
                            <span id="search-distance" style={{display: "flex", alignItems: "center"}}>
                                <input id="search-distance-left"
                                    className={(searchDistanceError) ? "search-distance-error":""}
                                    type="text" autoComplete="off" spellCheck="false"
                                    value={searchDistanceKm}
                                    onChange={event => handleValueChange(event, 'searchDistanceKm')}
                                />
                                <p>km&nbsp;</p>
                                <input id="search-distance-right" 
                                    className={(searchDistanceError) ? "search-distance-error":""}
                                    type="text" autoComplete="off" spellCheck="false"
                                    value={searchDistanceM}
                                    onChange={event => handleValueChange(event, 'searchDistanceM')}
                                />
                                <p>m</p>
                            </span>
                        </section>
                    ...
                    <div id="messages">
                        <section>
                            ...
                            {
                                (statusMsg === 1) ?
                                <p className="error-msg"><IoAlertCircle size={18}/>&nbsp;Please fill the required fields and try again</p>
                                :
                                (statusMsg === 2) ?
                                <p className="loading-msg"><IoTime size={18}/>&nbsp;Searching the database...</p>
                                :
                                (statusMsg === 3) ?
                                <p className="error-msg">Permission to use device geolocation has not been granted</p>
                                :
                                (statusMsg === 4) ?
                                <p className="error-msg">This feature is not supported by your browser</p>
                                :
