...
import { IoWarning, IoAlertCircle, IoCheckmarkCircle, IoTime } from 'react-icons/io5';
...
export class AddMenu extends Component {
    ...
    handleValueChange = (event, key) => {
        this.setState({ [key]: event.target.value },
            () => this.validateInput(key)); // Callback to validate the input
    };
    
    // Validate input
    validateInput = (key) => {
        const { locationName, locationType, locationDescription, error } = this.state;

        if (key === 'locationName') {
            if (locationName.length === 0 || locationName.length > 50 || !locationName.match("^$|^[A-Za-z0-9-äöåÄÖÅ() ]+$"))
                this.setState({ error: { ...error, locationNameError: true } });
            else
                this.setState({ error: { ...error, locationNameError: false } });
        }
        if (key === 'locationType') {
            if (locationType.length === 0 || locationType.length > 50 || !locationType.match("^$|^[A-Za-z0-9-äöåÄÖÅ ]+$"))
                this.setState({ error: { ...error, locationTypeError: true } });
            else
                this.setState({ error: { ...error, locationTypeError: false } });
        }
        if (key === 'locationDescription') {
            if (locationDescription.length > 250 || !locationDescription.match("^$|^[A-Za-z0-9-äöåÄÖÅ()/.,!?_ ]+$"))
    ...
    handleSetRating = (value) => {
        ...
        this.setState({ error: { ...this.state.error, locationRatingError: false } });
    };

    // Allows user to set coordinates by calling a parent function that activates map selection
    handleSetCoordinates = () => {
        this.props.startMapSelection();
        this.setState({ error: { ...this.state.error, locationCoordinateError: false } });
    };
    ...
    handleSubmit = () => {
        const { locationName, locationType, locationDescription, locationRating } = this.state;
        const { latitude, longitude } = this.props;
        let setNameError, setTypeError, setDescriptionError, setRatingError, setCoordinateError;
        
        if (locationName.length > 50 || locationName.length === 0 || !locationName.match("^$|^[A-Za-z0-9-äöåÄÖÅ() ]+$"))
            setNameError = true;
        else
            setNameError = false;
        
        if (locationType.length > 50 || locationType.length === 0 || !locationType.match("^$|^[A-Za-z0-9-äöåÄÖÅ() ]+$"))
            setTypeError = true;
        else
            setTypeError = false;
        ...
        if (setNameError || setTypeError || setDescriptionError || setRatingError || setCoordinateError)
            ...
            this.setState({
                ...
                axios.post('http://localhost:9000/api/locations',
                {
    ...
    render() {
        const { locationName, locationType, locationDescription, locationRating, loading, statusMsg } = this.state;
        const { locationNameError, locationTypeError, locationDescriptionError, locationRatingError, locationCoordinateError } = this.state.error;
        const { latitude, longitude, selectingFromMap, enableMapCentering, handleCheckboxChange } = this.props;
        const { handleValueChange, handleSetRating, handleSetCoordinates, handleSubmit, handleReset } = this;
                    ...
                    <div id="input-fields">
                        ...
                        <section>
                            ...
                            <input id="location-type"
                                ...
                                type="text" autoComplete="off" spellCheck="false" placeholder="e.g. Ice Hockey"
                                value={locationType}
                        ...
                        <section>
                            ...
                            <textarea cols="50" rows="5" id="location-description" 
                                className={(locationDescriptionError) ? "inputFieldError":""}
                                autoComplete="off" spellCheck="false" placeholder="Optional"
                                value={locationDescription}
                                onChange={event => handleValueChange(event, 'locationDescription')}
                            />
                        </section>
                        ...
                        <section>
                            ...
                            <button id={(selectingFromMap) ? "location-coordinates-active" : "location-coordinates"}
                    ...
                    <div id="messages">
                        <section>
                            <div className="status-msg">
                            {
                                ...
                                :
                                (statusMsg === 2) ?
                                <p className="loading-msg"><IoTime size={18}/>&nbsp;Adding location to database...</p>
                                :
                                (statusMsg === 3) ?
                                <p className="success-msg"><IoCheckmarkCircle size={18}/>&nbsp;Location added succesfully</p>
                                :
                ...
                return ( 
                    <label key={"star-" + ratingValue}>
                        ...
                        <FaStar 
                            className="star" 
                            size={40}
                            color={ratingValue <= (hover || locationRating) ? "#ffc107":"#e4e5e9"}
